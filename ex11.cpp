...
struct Binary_tree : Shape
{
  ...
  Binary_tree(int level, int radius);
  Binary_tree(int level, int radius, int vertical_distance);
  void draw_lines() const override;

private:
  int lvl, r = 10, s = 50;
  Lines lines;
  Point btroot{x_max() / 2 - r, r}; // store top-left corner
  void add_tree();
};
...
Binary_tree::Binary_tree(int level, int radius)
    : lvl{level}, r{radius}
{
  add_tree();
}
...
Binary_tree::Binary_tree(int level, int radius, int vertical_distance)
    : lvl{level}, r{radius}, s{vertical_distance}
{
  add_tree();
}
